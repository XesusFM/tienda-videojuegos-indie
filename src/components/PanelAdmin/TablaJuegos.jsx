            ...
            <table className="min-w-full table-fixed border border-gray-700">
                <thead>
                    <tr className="bg-gray-800 text-pink-400">
                        <th className="w-12 px-4 py-2 text-center cursor-pointer" onClick={() => handleSort("id")}>
                            ID
                            {sortField === "id" && (sortOrder === "asc" ? "▲" : "▼")}
                        </th>
                        <th className="w-32 px-4 py-2 text-center cursor-pointer" onClick={() => handleSort("titulo")}>
                            Título
                            {sortField === "titulo" && (sortOrder === "asc" ? "▲" : "▼")}
                        </th>
                        <th className="w-24 px-4 py-2 text-center">Imagen</th>
                        <th className="w-24 px-4 py-2 text-center cursor-pointer" onClick={() => handleSort("precio")}>
                            Precio
                            {sortField === "precio" && (sortOrder === "asc" ? "▲" : "▼")}
                        </th>
                        <th className="w-48 px-4 py-2 text-center cursor-pointer" onClick={() => handleSort("descripcion")}>
                            Descripción{" "}
                            {sortField === "descripcion" && (sortOrder === "asc" ? "▲" : "▼")}
                        </th>
                        <th className="w-32 px-4 py-2 text-center cursor-pointer" onClick={() => handleSort("categoria")}>
                            Categoría{" "}
                            {sortField === "categoria" && (sortOrder === "asc" ? "▲" : "▼")}
                        </th>
                        <th className="w-24 px-4 py-2 text-center cursor-pointer" onClick={() => handleSort("descuento")}>
                            Descuento{" "}
                    ...
                    {sortedJuegos.map((juego) => (
                        <tr key={juego.id} className="border-t border-gray-700">
                            ...
                            <td className="px-4 py-2 text-center">
                                <img src={juego.imagen} alt={juego.titulo} className="w-16 rounded mx-auto" />
                            </td>
                            ...
                            <td className="px-4 py-2 text-center">
                                ...
                                <button onClick={() => onSeleccionarJuego(juego)} className="bg-green-500 px-3 py-1 rounded text-white hover:bg-green-600 mr-2">
                                    Editar
                                </button>
                                <button onClick={() => handleEliminar(juego)} className="bg-red-500 px-3 py-1 rounded text-white hover:bg-red-600">
                                    Eliminar
                                </button>
                            </td>
                        </tr>
